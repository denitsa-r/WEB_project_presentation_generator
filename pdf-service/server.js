...
app.post('/generate-pdf', async (req, res) => {
    ...
    try {
        ...
        const pdfSize = (pdfBase64.length * 0.75) / 1024; // Size in KB
        const duration = Date.now() - startTime;
        
        console.log(`[PDF Generation] Success! Size: ${pdfSize.toFixed(2)} KB, Duration: ${duration}ms`);
...
app.post('/generate-pdf-batch', async (req, res) => {
    try {
        ...
        for (let i = 0; i < presentations.length; i++) {
            ...
            console.log(`[Batch PDF] Processing ${i + 1}/${presentations.length}: ${title}`);
        ...
        console.log(`[Batch PDF] Completed: ${successCount}/${presentations.length} successful`);
